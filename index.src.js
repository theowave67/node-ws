...
const UUID = process.env.UUID || '6d247c88-c54e-44dd-b02e-3db65f86e14c'; // 运行哪吒v1,在不同的平台需要改UUID,否则会被覆盖
...
const httpServer = http.createServer((req, res) => {
  if (req.url === '/') {
    ...
    const vlessURL = `vless://${UUID}@${HOST}:443?encryption=none&security=tls&sni=${DOMAIN}&fp=chrome&type=ws&host=${DOMAIN}&path=%2F${WSPATH}#${namePart}`;
    const trojanURL = `trojan://${UUID}@${HOST}:443?security=tls&sni=${DOMAIN}&fp=chrome&type=ws&host=${DOMAIN}&path=%2F${WSPATH}#${namePart}`;
